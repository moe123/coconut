	...
	try {
		...
		if (std::regex_match(slc_string, match, regex)) {
			if (match.size() == 4) {
				...
				sub_match = match[3];
				m_step = algorithm::to_numeric<std::uint16_t>(sub_match.str());
			}
		}
	}
...
islice::islice(std::int64_t start, std::int64_t stop, std::uint16_t step) :
	m_start(start),
	m_stop(stop),
	m_step(step)
{ /* NOP */ }
...
std::uint16_t islice::step() const
{ return m_step; }
...
void islice::set_step(std::uint16_t step)
{ m_step = step; }

#pragma mark -

bool islice::iszero() const
{ return ((m_start + m_stop) == 0); }

std::size_t islice::max() const
